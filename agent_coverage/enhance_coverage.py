...
from sonar.adapters import SonarPreflightError, get_adapter as get_sonar_adapter
...
class CoverageBaselineStep(BaseAgent):
    ...
    has no coverage-report tooling configured (JaCoCo for Java,
    karma-coverage/Vitest's coverage provider for TypeScript) -- without it
    every file reads 0% on Sonar and generated tests have no measurable
    effect, so there's nothing for this agent to verify; (2) capture the
    project's coverage % now, so the final report can show a real
    before/after delta. branch=source_branch (or the project default) --
    this run's own branch doesn't exist yet."""
    name: str = "coverage_baseline_step"

    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        s = ctx.session.state
        working_dir = s[sk.WORKING_DIR]
        adapter = get_sonar_adapter(s[sk.LANGUAGE], working_dir)

        if not adapter.has_coverage_tooling(working_dir):
            raise SonarPreflightError(
                "This project has no coverage-report tooling configured, so Sonar "
                "reports 0% coverage for every file and the coverage agent has no "
                "way to measure whether generated tests actually help.\n"
                f"{adapter.coverage_setup_instructions()}"
            )
...
class CoverageFileFixerStep(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        test_file_path = adapter.production_to_test_path(entry["file"])
...
class CoverageApplyAndVerifyStep(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        test_result = adapter.run_specific_tests(working_dir, [adapter.test_identifier(test_file)])
        retried = False
...
class CoverageQualityGateStep(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        my_files = set(s[sk.FILES_COMPLETED]) | {adapter.production_to_test_path(f) for f in s[sk.FILES_COMPLETED]}
...
class CoverageFinalVerifyStep(BaseAgent):
    ...
    async def _attempt_build_fix(
        ...
        adapter = get_adapter(s[sk.LANGUAGE], working_dir)
        prod_by_test = {adapter.production_to_test_path(f): f for f in s[sk.FILES_COMPLETED]}

        # parse_junit_failures yields "<classname>#<method>: <message>" --
        # take the class/suite name, match it to one of this run's test
        # files by basename-minus-extension. Best-effort (see the fallback
        # right below for when it finds nothing) -- exact for Java's
        # dotted FQCN vs Test.java basename, weaker for a Karma/Vitest
        # describe-block name vs a kebab-case *.spec.ts filename, which is
        # fine since a miss here just falls through to "every test file
        # this run wrote."
        targets: set[str] = set()
        for failure in failing_tests:
            simple = failure.split("#", 1)[0].strip().rsplit(".", 1)[-1]
            targets |= {tf for tf in prod_by_test if os.path.splitext(os.path.basename(tf))[0] == simple}
