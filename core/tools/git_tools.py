...
def resolve_source(
    source: str, source_type: str, workspace_root: str, github_token: str | None = None,
    ...
    if source_type == "github":
        ...
        #
        # --branch handles both cases in one step: checks out source_branch
        # directly if given (failing clearly, via _run's RuntimeError, if
        # that branch doesn't exist on origin), or is simply omitted to get
        # the remote's own default branch, same as before this option existed.
        clone_args = ["git", "-c", "core.autocrlf=false", *auth, "clone"]
        if source_branch:
