    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        if not (s.get("source") and s.get(sk.SOURCE_TYPE)):
            if len(ctx.session.events) <= 1:
                yield Event(
                    author=self.name,
                    content=types.Content(role="model", parts=[types.Part(text=shared_intake.COVERAGE_WELCOME_MESSAGE)]),
                )
                return
            ...
            yield Event(
                ...
                content=types.Content(role="model", parts=[types.Part(text=(
                    ...
                    f"{s['source']}{branch_note}. Starting the coverage enhancement now."
                ))]),
            )
