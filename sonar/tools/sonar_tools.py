...
def validate_connection(sonar_base_url: str, token: str) -> None:
    ...
    from ..adapters import SonarPreflightError
    try:
...
def get_metric_value(sonar_base_url: str, project_key: str, token: str, metric_key: str, branch: str | None) -> str | None:
    """GET /api/measures/component for a single project-wide metric, e.g.
    'coverage' or 'duplicated_lines_density' — used by the coverage/
    duplicate agents' report steps to show a real final number instead of
    ...
    isn't present (e.g. no analysis yet on this branch)."""
    return _get_measures(sonar_base_url, project_key, token, [metric_key], branch).get(metric_key)
